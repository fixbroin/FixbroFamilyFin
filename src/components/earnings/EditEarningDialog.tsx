...
export function EditEarningDialog({ earning, open, onOpenChange }: EditEarningDialogProps) {
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      ...
      date: earning.date.toDate(),
      isPrivate: earning.isPrivate,
    },
  });
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    setLoading(true);
    try {
      const earningRef = doc(db, "families", family.id, "earnings", earning.id);
      await updateDoc(earningRef, {
        ...values,
        date: Timestamp.fromDate(values.date),
        updatedAt: serverTimestamp(),
      });
      toast({ title: "Success", description: "Earning updated successfully." });
      onOpenChange(false);
    } catch (error) {
      toast({ variant: "destructive", title: "Error", description: "Could not update earning. Please try again." });
    } finally {
      setLoading(false);
    }
  }
