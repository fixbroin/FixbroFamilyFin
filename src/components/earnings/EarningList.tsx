...
import { useEarnings, useEarningCategories } from "@/hooks/useFamilyData";
...
export function EarningList() {
  ...
  const filteredEarnings = useMemo(() => {
    if (!user) return [];
    return earnings.filter(earning => !earning.isPrivate || earning.addedBy === user.uid);
  }, [earnings, user]);
  ...
  const loading = earningsLoading || categoriesLoading;
